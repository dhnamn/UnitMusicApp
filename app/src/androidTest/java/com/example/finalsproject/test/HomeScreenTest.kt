package com.example.finalsproject.test

//import android.annotation.SuppressLint
//import androidx.activity.ComponentActivity
//import androidx.compose.ui.test.junit4.createAndroidComposeRule
//import androidx.compose.ui.test.onNodeWithText
//import com.example.finalsproject.model.FetchStatus
//import com.example.finalsproject.model.Playlist
//import com.example.finalsproject.model.Song
//import com.example.finalsproject.ui.screen.ExploreSection
//import com.example.finalsproject.ui.screen.PlaylistSection
//import com.example.finalsproject.ui.screen.TopSongSection
//import com.example.finalsproject.ui.viewmodel.HomeScreenState
//import kotlinx.coroutines.flow.MutableStateFlow
//import org.junit.Rule
//import org.junit.Test
//
//class HomeScreenTest {
//    @get: Rule
//    val composeTestRule = createAndroidComposeRule<ComponentActivity>()
//
//    private var img = "data:image/png;base64,"
//
//    private val listSongs = listOf(
//        Song(1, "Song1", "Artist1", "album1", img, "genre1", 120, "01/01/2020", 53, 43, true),
//        Song(2, "Song2", "Artist2", "album2", img, "genre2", 140, "02/02/2020", 43, 17, true),
//        Song(3, "Song3", "Artist3", "album3", img, "genre3", 130, "03/03/2020", 33, 6, false),
//        Song(4, "Song4", "Artist4", "album4", img, "genre4", 110, "04/04/2020", 33, 14, false),
//        Song(5, "Song5", "Artist5", "album5", img, "genre5", 170, "05/05/2020", 13, 3, true))
//
//    private val listPlaylists = listOf(
//        Playlist(1, "playlist1", "description1", img, listSongs),
//        Playlist(2, "playlist2", "description2", img, listSongs),
//        Playlist(3, "playlist3", "description3", img, listSongs),
//        Playlist(4, "playlist4", "description4", img, listSongs),
//        Playlist(5, "playlist5", "description5", img, listSongs)
//    )
//
//    private val state = MutableStateFlow(
//        HomeScreenState(
//            topSongs = FetchStatus.Ready(listSongs),
//            exploreSongs = FetchStatus.Ready(listSongs),
//            playlists = FetchStatus.Ready(listPlaylists)
//        )
//    )
//
//    @SuppressLint("StateFlowValueCalledInComposition")
//    @Test
//    fun homeScreenTest_verifyTopSongSection() {
//        composeTestRule.setContent {
//            TopSongSection(
//                songs = state.value.topSongs,
//                navToLeaderboard = {},
//                onClickRetry = {}
//            )
//        }
////        composeTestRule.onNodeWithText("Top songs").assertExists()
////        composeTestRule.onNodeWithText("Song1").assertExists()
//    }
//
//    @SuppressLint("StateFlowValueCalledInComposition")
//    @Test
//    fun homeScreenTest_verifyPlaylistSection() {
//        composeTestRule.setContent {
//            PlaylistSection(
//                playlists = state.value.playlists,
//                onClickRetry = {},
//                onClickPlaylist = {}
//            )
//        }
//        composeTestRule.onNodeWithText("Our playlist picks").assertExists()
//        composeTestRule.onNodeWithText("playlist1").assertExists()
//    }
//
//    @SuppressLint("StateFlowValueCalledInComposition")
//    @Test
//    fun homeScreenTest_verifyExploreSection() {
//        composeTestRule.setContent {
//            ExploreSection(
//                songs = state.value.exploreSongs,
//                onClickRetry = {}
//            )
//        }
//        composeTestRule.onNodeWithText("Explore").assertExists()
//        composeTestRule.onNodeWithText("Song1").assertExists()
//    }
//}
//
